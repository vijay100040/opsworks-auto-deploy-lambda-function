...
module.exports = function(message, config, context) {
  ...
  function validateDeploymentStatus(jobData, command, callback) {
    dbHandler.getDeploymentStatus(jobData.config.appName, function(err, data) {
      ...
      allowProgress = allowProgress || process.env.DEBUG == 1;
  ...
  async.waterfall([
      ...
      function(data, callback) {
        ...
        dbHandler.updateDeploymentStatus(jobData.config.appName, utils.buildPipelineStatus(message.command, contextHolder.STATUS_INPROGRESS), jobData.lastVersionId, contextHolder.STATUS_RUNNING, 'NEW', message.command, callback);
      },
      function(callback) {
      ...
      function(data, callback) {
        dbHandler.updateDeploymentStatus(jobData.config.appName, utils.buildPipelineStatus(message.command, contextHolder.STATUS_INPROGRESS), jobData.lastVersionId+1, contextHolder.STATUS_RUNNING, data.DeploymentId, message.command, callback);
      }
    ],
    function(err, results) {
      if (err) {
