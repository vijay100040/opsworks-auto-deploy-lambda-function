			...
			if (data.Deployments.length > 0) {
				...
				async.waterfall([
						function (callback) {
							...
							// Insert Activity table data
							var inputParams = {};
							inputParams.message = message;
								if(pipelineStatus == helper.STATUS_HALTED) {
											inputParams.pipelineStatus = helper.STATUS_HALTED;
								} else if (pipelineStatus == helper.STATUS_FAILED) {
											inputParams.pipelineStatus = helper.STATUS_FAILED;
								} else if(pipelineStatus == helper.STATUS_SUCCESSFUL) {
											inputParams.pipelineStatus = helper.STATUS_SUCCESSFUL;
								}
