...
module.exports = function(message, config, context) {
  ...
  async.waterfall([
      ...
      function(data, callback) {
        ...
        dbHandler.updateDeploymentStatus(jobData.config.appName, utils.buildPipelineStatus(message.command, contextHolder.STATUS_INPROGRESS), jobData.lastVersionId, contextHolder.STATUS_RUNNING, 'NEW', callback);
      },
      function(callback) {
      ...
      function(data, callback) {
        dbHandler.updateDeploymentStatus(jobData.config.appName, utils.buildPipelineStatus(message.command, contextHolder.STATUS_INPROGRESS), jobData.lastVersionId+1, contextHolder.STATUS_RUNNING, data.DeploymentId, callback);
      }
    ],
    function(err, results) {
      if (err) {
