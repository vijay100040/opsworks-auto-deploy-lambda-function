...
function updateLock(appName, lockType, lockName, callback) {
  ...
  dynamodb.getItem(params, function(err, result) {
    ...
    var timesinceLastLockSeconds = moment().diff(moment(itemData.lock_datetime), 'seconds');
...
function updateLockWithRetry(appName, lockType, lockName, callback) {
  ...
  }, updateTask, function (err, results) {

    if(err) {
      console.log("failed to updateLock: n:%s, o:%s tries:%s", lockName, lockType, tryCount);
      return callback(err);
    } else {
      console.log("updateLock: n:%s, o:%s tries:%s", lockName, lockType, tryCount);
      return callback();
    }
  });
}
