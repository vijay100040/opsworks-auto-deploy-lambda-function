...
module.exports = function(jobData, message, context, fnCallback) {
  ...
  function getLayerForInstances(instances, upstreamData, layerData, callback) {
    ...
    instances.forEach(function(item) {
      ...
      if (upstreamData.servers.indexOf(publicDns) >= 0) {
        // console.log("instance layer %s %s", publicDns, item.LayerIds);
        ...
        if (!upstreamData.layerId) {
          ...
          // console.log("instance layer ID %s %s", publicDns, upstreamData.layerId);
        } else if (upstreamData.layerId != layerId) {
          console.log("Invalid State: Two instances in different layers. L1: %s L2:%s IP:%s", upstreamData.layerId, layerId, publicDns);
  ...
  function getLayerInstances(instances, layers, targetLayers, callback) {
    ...
    function(err, results) {
      ...
      async.waterfall([
          ...
          function(data, callback) {
            ...
            targetEnvConfig = decideTargetEnvSet(jobConfig, message, results.instances, results.layerData, results.upstreamData.production.productionEnvSet);
