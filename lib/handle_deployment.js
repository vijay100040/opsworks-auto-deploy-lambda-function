...
module.exports = function (message, config, context) {
	...
	function beginDeployment(command, targetEnvConfig, callback) {
		...
		opsworks.createDeployment(params, function (err, data) {
			if (err) {
				logger.debug("Deployment to OpsWorks failed. {%s}", err);
				return callback(err);
			}
			logger.debug("OpsWorks deployment %s triggered for command %s. ", data.DeploymentId, command);
			utils.publishMonitorMessage(config.monitoringTopicArn, "monitorDeployment", command, data.DeploymentId, function (err) {
	...
	var updateParams = {};
	updateParams.deploymentStatus = utils.buildPipelineStatus(message.command, contextHolder.STATUS_INPROGRESS);
	updateParams.pipelineStatus = contextHolder.STATUS_RUNNING;
	updateParams.deploymentId = jobData.deploymentId;
	updateParams.lastCommand = message.command;

	async.waterfall([
			function (callback) {
				logger.debug("Processing handle message for command %s", message.command);
			...
			function (data, callback) {
				...
				if (contextHolder.COMMANDS_IN_PIPELINE.indexOf(message.command) === 0) {
			...
			function (data, callback) {
				updateParams.deploymentId = data.DeploymentId;
				updateParams.currentVersion  = jobData.lastVersionId;
		...
		function (err) {
			if (err) {
				...
				if (jobData && jobData.dbUpdated) {
					updateParams.deploymentStatus  = utils.buildPipelineStatus(message.command, contextHolder.STATUS_FAILED);
					updateParams.currentVersion  = jobData.lastVersionId;
					updateParams.pipelineStatus = contextHolder.STATUS_NOTRUNNING;
					updateParams.deploymentId = jobData.deploymentId;
					dbHandler.updateDeploymentStatus(jobData.config.appName, updateParams,  utils.noopCallback);
				}
			}
			if (locked) {
				dbHandler.releaseLock(jobData.config.appName, "handleDeployment", function () {
					if (err) {
						...
						logger.debug("handleDeployment: Marking a failure. This may cause retries of the operation %s , err %s", message.command, err);
						context.fail(err);
						return;
					}
				});
