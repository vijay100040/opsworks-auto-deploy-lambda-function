...
function updatePipelineStatus(appName, options, callback) {

  incrementKeyCounter(options, function(err, data) {
  var params = {};
  ...
  var insertParams = {};
  insertParams.appName = appName;
  insertParams.activitySeq = result.Item.key_seq_counter;
  insertParams.deploymentSource = options.message.source;
  insertParams.pipelineStatus = options.pipelineStatus;
  insertParams.details = "triggered";
  insertParams.timeSpent = Date.now();
  insertParams.pipelineId = options.message.pipelineId;
  insertActivityData(insertParams, callback);
});
if(err) {
  return callback(err);
}
...
function incrementKeyCounter(options,callback) {
  var params = {};
  console.log(options.appName);
  params.TableName = ACTIVITY_SEQUENCE;
  params.Key = {
    "key_name": options.appName
  };
  dynamodb.getItem(params, function (err, data) {
    var last_key_seq_counter = data.Item.key_seq_counter;
    params.ExpressionAttributeValues = {
      ":updatedKeySeqCounter": last_key_seq_counter + 1,
      ":lastKeySeqCounter": last_key_seq_counter,
      ":appName":data.Item.key_name
    };
    params.UpdateExpression = "set key_seq_counter = :updatedKeySeqCounter";
    params.ConditionExpression = "key_seq_counter = :lastKeySeqCounter and key_name = :appName";
    dynamodb.updateItem(params, function(err,data) {
      if (err) {
        logger.debug("Had a problem while incrementing key counter %s", err);
        return callback(err);
      }
      callback(null,data.Item);
    });
...
function insertActivityData(options, callback) {
  var params = {
    ...
    Item:{
        ...
        "execution_time" : options.timeSpent,
...
dynamodb.putItem(params, function(err, data){
  ...
  logger.debug("Had a problem while increment activity data %s", err);
  return callback(err);
  }
  callback(data);
});
