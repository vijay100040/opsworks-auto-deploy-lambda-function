	...
	async.waterfall([
			...
			function (data, callback) {
				...
				if (helper.DEPLOYMENT_COMMAND_SEQ.indexOf(message.command) === 0) {
					...
					updateParams.pipelineId = uuid.v4();
				}

				var insertData = {};
				insertData.pipelineId = uuid.v4();
				insertData.deploymentBeginDatetime = Date.now();
				insertData.deploymentSource = message.source;
				insertData.pipelineStatus = helper.PIPELINE_STARTED;
				insertData.deploymentId = jobData.deploymentId;
				insertData.message = message.command;
