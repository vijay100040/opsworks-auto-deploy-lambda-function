	...
	function processMonitoringStatus(command, deployID, lastDeploymentData, callback) {
		...
		opsworks.describeDeployments(params, function (err, data) {
			...
			if (data.Deployments.length > 0) {
				...
				var commandIndex = helper.COMMANDS_IN_PIPELINE.indexOf(command);
				var nextCommand = "";
				...
				case "successful":
					...
					if (nextCommandIndex < helper.COMMANDS_IN_PIPELINE.length) {
						// There are still other commands to execute, mark pipeline as inprogress
						pipelineStatus = helper.STATUS_RUNNING;

						nextCommand = helper.COMMANDS_IN_PIPELINE[nextCommandIndex];
					} else if (nextCommandIndex === helper.COMMANDS_IN_PIPELINE.length) {
						...
						if(lastDeploymentData.deployment_queued_flag) {
							nextCommand = helper.COMMANDS_IN_PIPELINE[0];
						}
					}
					break;
				case "failed":
			...
			function (callback) {
				...
				dbHandler.getDeploymentStatus(config.appName, function (err, data) {
					...
					if (data.pipeline_status === helper.STATUS_NOTRUNNING) {
						var deploymentStatus = helper.buildPipelineStatus(helper.COMMANDS_IN_PIPELINE[helper.COMMANDS_IN_PIPELINE.length-1], helper.STATUS_SUCCESSFUL);
