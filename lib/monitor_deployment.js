...
module.exports = function(message, config, context) {
  ...
  console.log('Processing monitoring message for %s', deployID );
  ...
  function processMonitoringStatus(command, deployID, callback) {
    var params = {
      ...
      if (data.Deployments.length > 0) {
        ...
        async.waterfall([
            ...
            function(data, callback) {
              dbHandler.updateDeploymentStatus(config.appName, deploymentStatus, data.item_version, pipelineStatus, deployID, callback);
            },
            function(callback) {
              if (nextCommand) {
