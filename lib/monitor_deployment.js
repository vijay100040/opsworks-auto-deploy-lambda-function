...
module.exports = function(message, config, context) {
    ...
    opsworks.describeDeployments(params, function(err, data) {
      ...
      if (data.Deployments.length > 0) {
        ...
        async.waterfall([
            ...
            function(callback) {
              dbHandler.releaseLock(config.appName, callback);
            }
          ],
          callback
        );
  ...
  async.waterfall([
      function(callback) {
        dbHandler.acquireLock(config.appName, callback);
      },
      function(callback) {
        locked = true;
      ...
      function(callback) {
        dbHandler.releaseLock(config.appName, utils.noopCallback);
        locked = false;
      }
    ],
